...
class Enemy:
    def __init__(self, path):
        ...
        self.rect = self.image.get_rect(center=(path[0].get_map_x(), path[0].get_map_y()))
        self.path = path
        self.path_index = 0
        self.position = pygame.Vector2((path[0].get_map_x(), path[0].get_map_y()))  # Use vectors for smooth movement
        self.speed = 1

    def move(self):
        # Move the enemy smoothly along the path
        target_position = pygame.Vector2((self.path[self.path_index].get_map_x(), self.path[self.path_index].get_map_y()))
